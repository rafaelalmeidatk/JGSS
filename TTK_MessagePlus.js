...
(function ($) {
	...
	$.FadePadding = parseInt($.Params["Face Padding"]);
	...
	Game_Message.prototype.initialize = function() {
		...
		this._messageAddHandler = null;
	};

	Game_Message.prototype.setMessageAddHandler = function(handler) {
		this._messageAddHandler = handler;
	};
	...
	Game_Message.prototype.add = function(text) {
		...
		if (this._messageAddHandler != null) {
			this._messageAddHandler(this._texts);
		}
	};
	...
	Scene_Map.prototype.initialize = function() {
		...
		$gameMessage.setMessageAddHandler(this.onMessageAdd.bind(this));
	};

	Scene_Map.prototype.onMessageAdd = function(texts) {
		this._messageWindow.refreshSize(texts);
		this._messageWindow.refreshPosition();
		this._messageWindow.refreshBackground();
	};
	...
	Window_Message.prototype.refreshSize = function(texts) {
		...
		if ($.data.characterFocus < 0 && !$.data.characterFocusCurrent) {
	...
	Window_Message.prototype.refreshPosition = function() {
		if ($.data.characterFocus < 0 && !$.data.characterFocusCurrent) {
			...
			if ($.data.characterFocusCurrent)
				$.data.characterFocus = $gameMap.getInterpreter().eventId();

			if ($.data.characterFocus == 0)
				var pos = [$gamePlayer.screenX(), $gamePlayer.screenY()];
			else
				var pos = [$gameMap.event($.data.characterFocus).screenX(), $gameMap.event($.data.characterFocus).screenY()];
	...
	Window_Message.prototype.updateFaceAnimation = function() {
		if (this.hasFace() && $.afData.active && this._allowAfUpdate) {
			if ($.afData.delayTick > 0) {
				$.afData.delayTick--;
			} else {
				$.afTick++;
				if ($.afTick >= $.afData.tickMax) {
					if ($.afData.frameSide == 0) {
						$.afData.frame++;
						if ($.afData.frame >= $.afData.maxFrames || ($.afData.frame >= $.afData.speakFrames && this._textState)) {
							$.afData.frameSide = 1;
						}
						$.afTick = 0;
					}
						...
						if ($.afData.frame <= 0) {
							$.afData.frameSide = 0;
							if ($.afData.delay[1] > 0 && (this._textState == null || $.afData.speakY < 0)) {
								$.afData.delayTick =  ~~(Math.random() * ($.afData.delay[1] - $.afData.delay[0] + 1) + $.afData.delay[0]);
							}
						}
						$.afTick = 0;
					}
	...
	Window_Message.prototype.drawMessageFace = function() {
		...
		if ($.data.characterFocus >= 0)
			if ($.animatedFace)
				this._faceWindow.drawMessageFace(fileName, $.afData.frame, this._textState);
			else
				this._faceWindow.drawMessageFace($gameMessage.faceName(), $gameMessage.faceIndex(), this._textState);
		else {
			if ($.animatedFace)
				this.drawFace(fileName, $.afData.frame, 0, 0);
			else
	...
	Window_Message.prototype.newLineX = function() {
		return $gameMessage.faceName() === '' || $.data.characterFocus >= 0 ? 2 : 168;
	};
	...
	Window_Message.prototype.newPage = function(textState) {
		...
		if (this.hasFace() && $.data.characterFocus >= 0)
	...
	Window_Message.prototype.drawFace = function(faceName, faceIndex, x, y, width, height) {
		if ($.afData.active && $.afData.speakY >= 0 && !this._textState && $.afData.y < 0) {
		...
		if ($.afData.active && $.afData.speakY >= 0 && this._textState) {
			sx = faceIndex * pw;
			sy = ph * $.afData.speakY;
		} else if ($.afData.active && $.afData.y >= 0 && !this._textState) {
			sx = faceIndex * pw;
			sy = ph * $.afData.y;
		}
	...
	Window_Message_Name.prototype.refreshSize = function() {
		this.width = this.textWidth($.data.messageName) + 60 + (this.standardPadding() * 2);
	};
	...
	Window_Message_Name.prototype.refreshName = function() {
		...
		this.drawName($.data.messageName);
	};
	...
	Game_Interpreter.prototype.pluginCommand = function(command, args) {
		...
		if (command == "MessagePlus") {
			if (args[0] == "setBallon") {
				if (args[1] == "current") {
					$.data.characterFocusCurrent = true;
				} else {
					$.data.characterFocusCurrent = false;
					$.data.characterFocus = parseInt(args[1]);
				}
			}

			if (args[0] == "removeBallon") {
				$.data.characterFocusCurrent = false;
				$.data.characterFocus = -1;
			}

			if (args[0] == "setName")
				$.data.messageName = args[1];

			if (args[0] == "removeName")
				$.data.messageName = "";

			if (args[0] == "enableAnimatedFace") {
				$.afData.y = parseInt(args[1]) - 1;
				$.afData.maxFrames = parseInt(args[2]) - 1;
				if (args[3]) {
					$.afData.delay = args[3].split(',').map(Number);
				}
				else
					$.afData.delay = [0, 0];
				$.afData.active = true;
			...
			if (args[0] == "setSpeakAnimatedFace") {
				$.afData.speakY = parseInt(args[1]);
				$.afData.speakFrames = parseInt(args[2] - 1);
				$.afData.active = true;
			}
	...
	DataManager.makeSaveContents = function() {
		...
		contents.ktkMessagePlus_data = $.data;
		contents.ktkMessagePlus_afData = $.afData;
		return contents;
	};
	...
	DataManager.extractSaveContents = function(contents) {
		...
		$.data = contents.ktkMessagePlus_data;
		$.afData = contents.ktkMessagePlus_afData;
	};
})(TTK.MessagePlus);